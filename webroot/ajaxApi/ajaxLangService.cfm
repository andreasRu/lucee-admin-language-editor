<cfscript>
	...
	if( structKeyExists( url, "method" )){
		...
		if( url.method == "getChatGPTPrompt" ){
			...
			result["error"]=0;
			result["success"]=true;
			result["contentForHtmlOutput"]= "<textarea name=""jsonEditor"" id=""jsonEditor"" style=""white-space: pre;overflow-nowrap: normal;overflow-x: scroll;width:80vw;max-width:900px;height:40vh;tab-size: 2;"">" & encodeForHTML( jsonSnippet ) & "</textarea>";
			LangEditorService.outputAsJson( result );

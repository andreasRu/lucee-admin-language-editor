...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = (langUpdater);
...
const myAjaxUtils = {
  buildPayLoad: (ajaxURL, httpMethod, selectorFormFields, selectorContentResult, jqueryHTMLCommand, callback) => {
    ...
    if (httpMethod == "POST") {
      ...
      $selector.each((index, obj) => {
        ...
        if (itemsProcessed === $selector.length) {
          myAjaxUtils.sendAjaxCommand(ajaxURL, formMethod, formData, callback).done(function (result) {
  ...
  validateAndsendAjaxForm: (ajaxURL, selectorFormFields, selectorContentResult, jqueryHTMLCommand, callback) => {
    let itemsProcessed = 0;
    let $selector = jquery__WEBPACK_IMPORTED_MODULE_0___default()(selectorFormFields);
    let allItemsOK = true;
    ...
    $selector.each((index, obj) => {
      ...
      if (jquery__WEBPACK_IMPORTED_MODULE_0___default()(obj).attr('data-isObligatory') == 'true') {
        ...
        if (itemsProcessed === $selector.length) {
          if (allItemsOK) {
            jquery__WEBPACK_IMPORTED_MODULE_0___default()('#loadingSpinner').show();
            myAjaxUtils.buildPayLoad(ajaxURL, 'POST', selectorFormFields, selectorContentResult, jqueryHTMLCommand, callback);
          } else {
            return false;
          }
        }
...
/******/ })()
;
//# sourceMappingURL=jsbundle.js.map?hash=dfe95cf94e9c11303dd7
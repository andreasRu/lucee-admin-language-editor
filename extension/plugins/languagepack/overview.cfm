    ...
    <form action="#request.self#?action=languagepack.switchLanguage" method="post">
        <select name="lang">
            <cfloop collection="#app.availableLanguages#" index="key">
                <option value="#key#" <cfif key==session.lucee_admin_lang>selected</cfif>>#encodeForHTML( app.availableLanguages[key] )#</option>
            </cfloop>
        </select>
        <input type="hidden" name="loadLanguages" value="execute">
        <div>
